...
extension UIView {
    ...
    var dragBlast: Bool {
        set {
            ...
            if newValue {
                ...
                let panGR = UIPanGestureRecognizer.init(target: self, action: #selector(dragPanAction(pan:)))
    ...
    @objc func tapBlastAction(tap: UITapGestureRecognizer) {
        if tapBlast {
            ...
            if isFragment { // 粒子动画
                self.boomCellsInGesture(gesture: tap)
            } else { // 图片的帧动画
                self.aViewBlastEffect(gesture: tap)
            }
        } else {
            transform = CGAffineTransform.init(scaleX: 1.1, y: 1.1)
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.2, initialSpringVelocity: 0, options: UIViewAnimationOptions.curveLinear, animations: {
    ...
    // MARK: - 图片的帧动画-爆炸
    func aViewBlastEffect(gesture: UIGestureRecognizer) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + DispatchTimeInterval.milliseconds(Int(0.5 * 1000))) { [weak self] in
            ...
            if let gestureView = gesture.view {
                ...
                if self!.translatesAutoresizingMaskIntoConstraints {
                    ...
                    // 创建左边约束
                    let leftLc = NSLayoutConstraint.init(item: self!, attribute: NSLayoutAttribute.left, relatedBy: NSLayoutRelation.equal, toItem: self?.resetSuperView, attribute: NSLayoutAttribute.left, multiplier: 1.0, constant: (belowFloorPoint.x) - R)
                    self?.resetSuperView.addConstraint(leftLc)
                    // 创建顶部约束
                    let topLc = NSLayoutConstraint.init(item: self!, attribute: NSLayoutAttribute.top, relatedBy: NSLayoutRelation.equal, toItem: self?.resetSuperView, attribute: NSLayoutAttribute.top, multiplier: 1.0, constant: (belowFloorPoint.y) - R)
    ...
    func boomCellsInGesture(gesture: UIGestureRecognizer) {
        let rowClocn = 3
        ...
        self.cellAnimations(cells: boomCells, gesture: gesture)
    }
    func cellAnimations(cells: [CALayer], gesture: UIGestureRecognizer) {
        for j in 0 ..< cells.count {
            ...
            pathAnimation.path = self.makeRandomPath(alayer: shape, index: CGFloat(j)).cgPath
            pathAnimation.timingFunctions = [CAMediaTimingFunction.init(name: kCAMediaTimingFunctionEaseOut)]
    ...
    // MARK: - 设置爆炸的碎片路径
    func makeRandomPath(alayer: CALayer, index: CGFloat) -> UIBezierPath {
    ...
    // MARK: 拖拽方法
    @objc func dragPanAction(pan: UIPanGestureRecognizer) {
        let window = UIApplication.shared.keyWindow!
        ...
        case .changed:
            ...
            var scale = 1 - centerDistance/(MAXMultiple*pan.view!.bounds.size.height)
        ...
        case .ended:
            ...
            if distance > MAXMultiple*pan.view!.bounds.size.height {
                ...
                if self.isFragment { // 粒子效果
                    self.boomCellsInGesture(gesture: pan)
                } else { // 图片的帧动画
                    self.aViewBlastEffect(gesture: pan)
                }
            } else {
                self.circle1?.isHidden = true
                self.shapeLayer?.path = nil
